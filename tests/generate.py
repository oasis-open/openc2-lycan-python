...
from openc2 import (
    parse, Command, Args, Payload, Device, DomainName, EmailAddress, IPv4Connection, 
    Response, File, Artifact, Process, Features, InternationalizedDomainName, 
    InternationalizedEmailAddress, IPv4Address, IPv6Address, IPv6Connection,
    MACAddress, IRI, Properties, URI
)
...
cmd=Command(action="stop",
            target=Artifact(payload=Payload(bin='YmluIGRhdGE='),
                hashes=hashes,
...
#Artifact (update)
cmd=Command(action="update",
            target=a)
with open('Artifact4.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Device
cmd=Command(action="allow",
            target=Device(hostname="device hostname",
...
#Domain Name
cmd = Command(action="cancel",
            target=DomainName(domain_name="Domain name"))
with open('DomainName.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Email Address
cmd = Command(action="copy",
            target=EmailAddress(email_addr="Email address"))
...
cmd = Command(action="deny",
            target=InternationalizedDomainName(idn_domain_name="IDN Domain name"))
with open('IdnDomainName.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#IdnEmailAddress
cmd = Command(action="detonate",
            target=InternationalizedEmailAddress(idn_email_addr="IDN Email address"))
with open('IdnEmailAddress.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Ipv4Connection
cmd = Command(action="investigate",
            target=IPv4Connection(src_addr="10.0.0.0/24",
                            src_port=8443,
...
cmd = Command(action="locate",
            target=IPv4Address(ipv4_net="10.0.0.0/24"))
with open('Ipv4Net.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Ipv6Connection
cmd = Command(action="query",
            target=IPv6Connection(src_addr="AE:00:E4:F1:04:65/24",
                            src_port=8443,
...
#MacAddress
cmd = Command(action="restart",
            target=MACAddress(mac_addr='VGhpcyBpcyBteSBtYWMgYWRkcmVzcw=='))
with open('MacAddress.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Parent
parent=Process(pid=43521,name="Process parent name",cwd="Process parent CWD")
cmd = Command(action="restore",
            target=Process(pid=12354,
                        name="Process name",
                        cwd="Process CWD",
                        executable=f,
                        parent=parent,
...
#Properties
cmd = Command(action="set",
            target=URI(uri="www.myuri.com"))
with open('URI.json', 'w') as OUT:
    OUT.write(cmd.serialize(pretty=True)+'\n')

#Properties
cmd = Command(action="scan",
            target=Properties(properties=["Tag1","Tag2","Tag3","Tag4"]))
